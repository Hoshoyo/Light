...
Ast* ast_create_expr_sizeof(Type_Instance* type, Scope* scope, Token* directive_token) {
	...
	d->expr_directive.type = EXPR_DIRECTIVE_SIZEOF;
	d->expr_directive.type_expr = type;
	d->expr_directive.token = directive_token;
...
Ast* ast_create_expr_typeof(Ast* expr, Scope* scope, Token* directive_token) {
	...
	d->expr_directive.type = EXPR_DIRECTIVE_TYPEOF;
	d->expr_directive.expr = expr;
	d->expr_directive.token = directive_token;
...
Ast* ast_create_decl_variable(Token* name, Scope* scope, Ast* assignment, Type_Instance* var_type, u32 flags) {
	...
	dv->decl_variable.flags = flags | (scope->level == 0) ? DECL_VARIABLE_DATA_SEGMENT : DECL_VARIABLE_STACK;
	...
	dv->decl_variable.field_index = 0;
